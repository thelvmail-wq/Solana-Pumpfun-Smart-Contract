...
pub struct CurveConfiguration {
    ...
    pub antivamp_signer: Pubkey,        // NEW: Ed25519 signer for source lock verification
}

impl CurveConfiguration {
    pub const SEED: &'static str = "CurveConfiguration";
    // 8 (disc) + 8 (f64) + 32 + 32 + 32 = 112
    ...
    pub fn new(fees: f64, protocol_wallet: Pubkey, airdrop_wallet: Pubkey) -> Self {
        Self {
            ...
            protocol_wallet,
            airdrop_wallet,
            antivamp_signer: Pubkey::default(),  // Default to zeroed — set via update_antivamp_signer
        }
    }
}
...
pub struct LiquidityPool {
    ...
    pub graduation_ts: i64,              // Unix timestamp when graduation triggered (0 if not graduated)
    pub meteora_pool: Pubkey,            // Meteora pool address (Pubkey::default if not migrated yet)
    pub migration_complete: bool,        // True after bot confirms Meteora pool creation
}

impl LiquidityPool {
    pub const POOL_SEED_PREFIX: &'static str = "liquidity_pool";
    // Original: 8 + 32 + 32 + 8 + 8 + 8 + 1 + 8 + 1 + 8 + 32 + 8 + 16 = 170
    // NEW fields: + i64 (8) + Pubkey (32) + bool (1) = +41
    // Total: 211
    // But we keep the +16 padding from original = 211
    pub const ACCOUNT_SIZE: usize = 8 + 32 + 32 + 8 + 8 + 8 + 1 + 8 + 1 + 8 + 32 + 8 + 16 + 8 + 32 + 1;
...
impl<'info> LiquidityPoolAccount<'info> for Account<'info, LiquidityPool> {
    ...
    fn swap(&mut self, _config: &Account<'info, CurveConfiguration>, t1: (&mut Account<'info, Mint>, &mut Account<'info, TokenAccount>, &mut Account<'info, TokenAccount>), t2: (&mut Account<'info, Mint>, &mut AccountInfo<'info>, &mut Signer<'info>), amount: u64, style: u64, bump: u8, auth: &Signer<'info>, tp: &Program<'info, Token>, sp: &Program<'info, System>) -> Result<()> {
        ...
        if style == 1 {
            ...
            let amount_out = convert_from_float(out_f, 9u8);
            // Max wallet check — UPDATED anti-snipe windows
            let clock = Clock::get()?;
            let elapsed = clock.unix_timestamp.saturating_sub(self.launch_timestamp);
            let total_tokens = t1.0.supply;
            let max_bps: Option<u64> = if elapsed < 120 {
                Some(150)       // 0-2min: 1.5% max
            } else if elapsed < 300 {
                Some(250)       // 2-5min: 2.5% max
            } else if elapsed < 600 {
                Some(500)       // 5-10min: 5% max
            } else {
                None            // 10min+: unlimited
            };
            ...
            if self.should_graduate() {
                ...
                msg!("GRADUATED! Total SOL raised: {:?}. Timestamp: {:?}", self.total_sol_raised, self.graduation_ts);
            }
        }
        Ok(())
    }
