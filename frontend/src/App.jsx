    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(30px)"}} onClick={onClose}>
      <div style={{background:C.sheet,borderRadius:"10px 10px 0 0",width:"100%",maxWidth:520,border:`1px solid ${C.border}`,borderBottom:"none",maxHeight:"92vh",overflowY:"auto",animation:"slideUp 0.28s cubic-bezier(0.34,1.56,0.64,1)"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"10px 20px 36px"}}>
          ...
          <Btn onClick={()=>{if(!ready||deployBlocked)return;setState("loading");(async()=>{try{const provider=window?.solana;const {Keypair,SystemProgram,Transaction:SolTx}=await import("@solana/web3.js");const {createInitializeMint2Instruction,TOKEN_PROGRAM_ID,MINT_SIZE,getMinimumBalanceForRentExemptMint}=await import("@solana/spl-token");const mintKp=Keypair.generate();const lamports=await getMinimumBalanceForRentExemptMint(connection);const createMintIxs=[SystemProgram.createAccount({fromPubkey:provider.publicKey,newAccountPubkey:mintKp.publicKey,space:MINT_SIZE,lamports,programId:TOKEN_PROGRAM_ID}),createInitializeMint2Instruction(mintKp.publicKey,9,provider.publicKey,provider.publicKey)];console.log("WALLET pubkey:",provider.publicKey.toBase58());for(let attempt=0;attempt<3;attempt++){try{const {Keypair:Kp2,SystemProgram:Sys2,Transaction:Tx2}=await import("@solana/web3.js");const {createInitializeMint2Instruction:initMint2,TOKEN_PROGRAM_ID:TPid,MINT_SIZE:MSz,getMinimumBalanceForRentExemptMint:getRent}=await import("@solana/spl-token");const {ComputeBudgetProgram:CBP}=await import("@solana/web3.js");const mk=Kp2.generate();const lam=await getRent(connection);const regTx=await buildDeployTx(provider.publicKey,mk.publicKey.toString(),form.ticker||"TEST",null,null);const fresh=new Tx2();fresh.add(CBP.setComputeUnitLimit({units:400000}));fresh.add(CBP.setComputeUnitPrice({microLamports:10000}));fresh.add(Sys2.createAccount({fromPubkey:provider.publicKey,newAccountPubkey:mk.publicKey,space:MSz,lamports:lam,programId:TPid}));fresh.add(initMint2(mk.publicKey,9,provider.publicKey,provider.publicKey));regTx.instructions.forEach(ix=>fresh.add(ix));const {blockhash,lastValidBlockHeight}=await connection.getLatestBlockhash("confirmed");fresh.recentBlockhash=blockhash;fresh.feePayer=provider.publicKey;fresh.partialSign(mk);const signed=await provider.signTransaction(fresh);const sig=await connection.sendRawTransaction(signed.serialize(),{skipPreflight:true,maxRetries:10});console.log("TX sent (attempt "+(attempt+1)+"):",sig);await connection.confirmTransaction({signature:sig,blockhash,lastValidBlockHeight},"confirmed");console.log("CONFIRMED");setState("done");break;}catch(retryErr){console.log("Attempt "+(attempt+1)+" failed:",retryErr.message);if(attempt===2)throw retryErr;await new Promise(r=>setTimeout(r,2000));}}).catch(e=>console.error("fetch tokens error:",e));},[]);
