    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(30px)"}} onClick={onClose}>
      <div style={{background:C.sheet,borderRadius:"10px 10px 0 0",width:"100%",maxWidth:520,border:`1px solid ${C.border}`,borderBottom:"none",maxHeight:"92vh",overflowY:"auto",animation:"slideUp 0.28s cubic-bezier(0.34,1.56,0.64,1)"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"10px 20px 36px"}}>
          ...
          <Btn onClick={()=>{if(!ready||deployBlocked)return;setState("loading");(async()=>{try{const provider=window?.solana;const {Keypair,SystemProgram,Transaction:SolTx}=await import("@solana/web3.js");const {createInitializeMint2Instruction,TOKEN_PROGRAM_ID,MINT_SIZE,getMinimumBalanceForRentExemptMint}=await import("@solana/spl-token");const mintKp=Keypair.generate();const lamports=await getMinimumBalanceForRentExemptMint(connection);const createMintIxs=[SystemProgram.createAccount({fromPubkey:provider.publicKey,newAccountPubkey:mintKp.publicKey,space:MINT_SIZE,lamports,programId:TOKEN_PROGRAM_ID}),createInitializeMint2Instruction(mintKp.publicKey,9,provider.publicKey,provider.publicKey)];console.log("WALLET pubkey:",provider.publicKey.toBase58());const registryTx=await buildDeployTx(provider.publicKey,mintKp.publicKey.toString(),form.ticker||"TEST",null,null);const {ComputeBudgetProgram}=await import("@solana/web3.js");const tx=new SolTx();tx.add(ComputeBudgetProgram.setComputeUnitLimit({units:400000}));tx.add(ComputeBudgetProgram.setComputeUnitPrice({microLamports:10000}));createMintIxs.forEach(ix=>tx.add(ix));registryTx.instructions.forEach(ix=>tx.add(ix));const {blockhash}=await connection.getLatestBlockhash();tx.recentBlockhash=blockhash;tx.feePayer=provider.publicKey;for(let attempt=0;attempt<3;attempt++){try{const {blockhash:bh,lastValidBlockHeight}=await connection.getLatestBlockhash("confirmed");tx.recentBlockhash=bh;tx.signatures=[];tx.partialSign(mintKp);const signed=await provider.signTransaction(tx);const sig=await connection.sendRawTransaction(signed.serialize(),{skipPreflight:true,maxRetries:10});console.log("TX sent (attempt "+(attempt+1)+"):",sig);await connection.confirmTransaction({signature:sig,blockhash:bh,lastValidBlockHeight},"confirmed");console.log("CONFIRMED");setState("done");break;}catch(retryErr){console.log("Attempt "+(attempt+1)+" failed:",retryErr.message);if(attempt===2)throw retryErr;await new Promise(r=>setTimeout(r,2000));}}}catch(e){console.error("Deploy error:",e);alert(e&&e.message?e.message:JSON.stringify(e));setState("idle");}})();}} full color={C.accent} loading={state==="loading"} disabled={!ready||deployBlocked||state==="loading"}>
...
function TabFeed({tokens, onSelect}) {
  ...
  const tabs = [
    {id:"all",    label:"All",         color:C.text,   tokens: [...tokens].sort((a,b)=>(b.isOnChain?1:0)-(a.isOnChain?1:0))},{id:"new",    label:"New",         color:C.blue,   tokens: [...tokens].filter(t=>t.elapsed<=90).sort((a,b)=>a.elapsed-b.elapsed)},
