    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(30px)"}} onClick={onClose}>
      <div style={{background:C.sheet,borderRadius:"10px 10px 0 0",width:"100%",maxWidth:520,border:`1px solid ${C.border}`,borderBottom:"none",maxHeight:"92vh",overflowY:"auto",animation:"slideUp 0.28s cubic-bezier(0.34,1.56,0.64,1)"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"10px 20px 36px"}}>
          ...
          <Btn onClick={()=>{if(!ready||deployBlocked)return;setState("loading");(async()=>{try{const provider=window?.solana;const {Keypair,SystemProgram,Transaction:SolTx}=await import("@solana/web3.js");const {createInitializeMint2Instruction,TOKEN_PROGRAM_ID,MINT_SIZE,getMinimumBalanceForRentExemptMint}=await import("@solana/spl-token");const {ComputeBudgetProgram}=await import("@solana/web3.js");const mk=Keypair.generate();const lam=await getMinimumBalanceForRentExemptMint(connection);const tx1=new SolTx();tx1.add(SystemProgram.createAccount({fromPubkey:provider.publicKey,newAccountPubkey:mk.publicKey,space:MINT_SIZE,lamports:lam,programId:TOKEN_PROGRAM_ID}));tx1.add(createInitializeMint2Instruction(mk.publicKey,9,provider.publicKey,provider.publicKey));const bh1=await connection.getLatestBlockhash("confirmed");tx1.recentBlockhash=bh1.blockhash;tx1.feePayer=provider.publicKey;tx1.partialSign(mk);const s1=await provider.signTransaction(tx1);const sig1=await connection.sendRawTransaction(s1.serialize(),{skipPreflight:true,maxRetries:10});console.log("Mint TX:",sig1);await connection.confirmTransaction({signature:sig1,blockhash:bh1.blockhash,lastValidBlockHeight:bh1.lastValidBlockHeight},"confirmed");console.log("Mint created");const {TransactionInstruction:TxIx,PublicKey:PK}=await import("@solana/web3.js");const PID=new PK("9cuFeeHRpr3yfjzeHLm84z95JPGaRgASwV4YY7PaMtkx");const tkr=form.ticker||"TEST";const enc=new TextEncoder();const thBuf=new Uint8Array(await crypto.subtle.digest("SHA-256",enc.encode(tkr.trim().toUpperCase())));const tBuf=Buffer.from(thBuf);const iH=Buffer.alloc(32);const dH=Buffer.alloc(32);const tR=Buffer.alloc(16);Buffer.from(tkr.slice(0,16)).copy(tR);const[reg]=PK.findProgramAddressSync([Buffer.from("token_registry"),mk.publicKey.toBuffer()],PID);const[tl]=PK.findProgramAddressSync([Buffer.from("ticker_lock"),tBuf],PID);const[il]=PK.findProgramAddressSync([Buffer.from("image_lock"),iH],PID);const[dl]=PK.findProgramAddressSync([Buffer.from("identity_lock"),dH],PID);const disc=Buffer.from("ec4062c2843c7324","hex");const ixD=Buffer.alloc(120);disc.copy(ixD,0);tBuf.copy(ixD,8);iH.copy(ixD,40);dH.copy(ixD,72);tR.copy(ixD,104);const rIx=new TxIx({keys:[{pubkey:reg,isSigner:false,isWritable:true},{pubkey:tl,isSigner:false,isWritable:true},{pubkey:il,isSigner:false,isWritable:true},{pubkey:dl,isSigner:false,isWritable:true},{pubkey:mk.publicKey,isSigner:false,isWritable:true},{pubkey:provider.publicKey,isSigner:true,isWritable:true},{pubkey:SystemProgram.programId,isSigner:false,isWritable:false}],programId:PID,data:ixD});const tx2=new SolTx();tx2.add(ComputeBudgetProgram.setComputeUnitLimit({units:400000}));tx2.add(ComputeBudgetProgram.setComputeUnitPrice({microLamports:10000}));tx2.add(rIx);const bh2=await connection.getLatestBlockhash("confirmed");tx2.recentBlockhash=bh2.blockhash;tx2.feePayer=provider.publicKey;const {signature:sig2}=await provider.signAndSendTransaction(tx2,{skipPreflight:true});console.log("Registry TX:",sig2);await connection.confirmTransaction({signature:sig2,blockhash:bh2.blockhash,lastValidBlockHeight:bh2.lastValidBlockHeight},"confirmed");console.log("DEPLOY COMPLETE");setState("done");}catch(e){console.error(e);alert(e.message);setState("idle");}})();}} full color={C.accent} loading={state==="loading"} disabled={!ready||deployBlocked||state==="loading"}>
